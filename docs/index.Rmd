---
title: "Team 4 Rough Draft Code Walkthrough"
output: 
  html_document:
    toc: yes
    toc_float:
...
options(cache = TRUE) # also runs faster
...
library(rlist)       # cross-machine interchange of list objects
...
tibble(v_codes, v_adm, v_desc) %>%
  kbl(caption = 'Table 2. NYC HPD Housing Maintenance Violation Categories Selected for Study',
...
Adjust data storage to work with did package
...
violations %>% 
  ungroup() %>% 
  mutate(cohort = factor(cohort)) %>%
  select(-c(geoid, inspection_yr_mo_num, tract_treat_zip, inspection_yr_mo, rtc_treat_date, treat_yr_mo, treat_yr_mo_num, treat_yr_mo_zero)) %>% 
  st(group = 'cohort',
     group.test = T,
     col.align = 'center',
     labels = labels,
     title = 'Table 1. Summary Statistics by Tract-Level RTC Treatment Cohort: NYC Renter-Occupied Units, January 2013 - May 2019',
     out = 'kable') %>% 
  kable_styling(full_width = F, 
                html_font = "Roboto Condensed",
                bootstrap_options = c("condensed", "striped"))
...
Table 3. Number of Months Treated, by RTC Cohort
...
violations %>% 
  ...
  kable(
    caption = 'Table 3. NYC Right-to-Counsel Treatment Waves Observed in 2013 - 2019 Study Period',
...
Table 4. Key Covariate Summaries, by Cohort (to highlight heterogeneity)
...
violations %>% 
  ...
  knitr::kable(digits = 2,
               caption = 'Table 4. Selected Potential Covariates by RTC Treatment Cohort: NYC Renter-Occupied Units, January 2013 - May 2019',
...
```{r}
ggdid(est_test,
      title = "Group-Time Average Treatment Effects of Right-to-Counsel on Landlord Maintenance Violations, 2013-2020 (No Covariates)",
      grtitle = "Received UA",
      xgap = 11)
```
...
### CS-DiD: With covariates (Tract-Level)

In addition to introducing the race-based covariates above (i.e. "controlling for" race), we subset New York City census tracts within each treatment cohort by U.S. Census-defined racial majority. We theorize that treatment effects vary with racial segregation, and opt to subset since CS-DiD does not allow for interaction terms.  
...
#### Group-Time ATE, All Tracts, Controlling for Treatment Selection Criteria
...
#### Dynamic Event Study with Plot (All Tracts)
...
#### Overall Effect of Participating in Treatment (All Tracts)
...
#### CS-DiD, Subset of Tracts With >50% Black Renter-Occupied Householders

```{r}
est_all <- att_gt(yname = "n_violations_per_1k_units",  
                       gname = "treat_yr_mo_num",
                       idname = "geoid",
                       tname = "inspection_yr_mo_num",
                       xformla = ~ evict_rate_17 + rs_rate_17 + pct_pov_17, 
                       ...
                       allow_unbalanced_panel = TRUE,
                       est_method = "dr") 
group_fx_all <- aggte(est_all, type = "group")
summary(group_fx_all)

est_w_race_covar <- att_gt(yname = "n_violations_per_1k_units",  
                       gname = "treat_yr_mo_num",
                       idname = "geoid",
                       tname = "inspection_yr_mo_num",
                       ...
                       allow_unbalanced_panel = TRUE,
                       est_method = "dr") 
group_fx_w_race_covar <- aggte(est_w_race_covar, type = "group")
summary(group_fx_w_race_covar)

est_bl <- att_gt(yname = "n_violations_per_1k_units",  
                       gname = "treat_yr_mo_num",
                       idname = "geoid",
                       tname = "inspection_yr_mo_num",
                       xformla = ~ evict_rate_17 + rs_rate_17 + pct_pov_17, 
                       ...
                       allow_unbalanced_panel = TRUE,
                       est_method = "dr") 
group_fx_bl <- aggte(est_bl, type = "group")
summary(group_fx_bl)
```
...
```{r}
t4a <- tidy(group_fx_all) %>% 
  select(-c(type, term, point.conf.low, point.conf.high)) %>% 
  mutate(group = str_replace_all(group, c("Average" = "All Groups", "2017.58" = "Cohort 1", "2017.75" = "Cohort 2", "2018.83" = "Cohort 3")))

t4b <- tidy(group_fx_bl) %>% 
  select(-c(type, term, point.conf.low, point.conf.high)) %>% 
  mutate(group = str_replace_all(group, c("Average" = "All Groups", "2017.58" = "Cohort 1", "2017.75" = "Cohort 2", "2018.83" = "Cohort 3")))

t4c <- tidy(group_fx_w_race_covar) %>% 
  select(-c(type, term, point.conf.low, point.conf.high)) %>% 
  mutate(group = str_replace_all(group, c("Average" = "All Groups", "2017.58" = "Cohort 1", "2017.75" = "Cohort 2", "2018.83" = "Cohort 3")))
```
...
knitr::kables(
  list(
    kable(
      ...
      valign = 't') %>% 
      add_header_above(c('Group', 'ATT', 'Std. Error', '95% Confidence Band' = 2)) %>%
      add_header_above(c('Covariates: Eviction Rate, Rent Stabilization Rate, Poverty Rate' = 4)) %>%
      add_header_above(c(' ', 'All Tract Neighborhoods' = 3)) %>%
      add_footnote(c("Signif. codes: '*' = confidence band does not cover 0", "Control Groups: Not Yet Treated, Anticipation Periods: 0", "Estimation Method: Doubly Robust"), notation = "none") %>% 
      kable_styling(),
    kable(t4c,
          col.names = NULL,
          ...
          valign = 't') %>% 
      add_header_above(c('Group', 'ATT', 'Std. Error', '95% Confidence Band' = 2)) %>%
     add_header_above(c('Covariates: Eviction Rate, Rent Stabilization Rate, Poverty Rate, % Black Renter Householders' = 4)) %>%
      add_header_above(c(' ', 'All Tract Neighborhoods' = 3)) %>%
      kable_styling(),
    kable(t4b,
          col.names = NULL,
      ...
      add_header_above(c('Group', 'ATT', 'Std. Error', '95% Confidence Band' = 2)) %>%
      add_header_above(c('Covariates: Eviction Rate, Rent Stabilization Rate, Poverty Rate' = 4)) %>%
      add_header_above(c(' ', '>50% Black Tract Neighborhoods' = 3)) %>%
      kable_styling()
    
  ),
  caption = 'Table 6. Aggregated Group-Specific Treatment Effects'
) %>% 
  kable_styling(full_width = F, 
                  html_font = "Roboto Condensed")
```
...
Callaway and Sant'Anna propose the Cramer von Mises or Kolgorov-Smirnov-type integrated moments test as one tool for assessing whether the conditional parallel trends assumption might hold for all groups in all pre-treatment periods. See https://search.r-project.org/CRAN/refmans/did/html/conditional_did_pretest.html and Section 4 of Callaway & Sant'Anna (2020).

Since these tests are extremely memory-heavy and take approximately three to four hours to run per model (at the time of this writing), we include just the code used to generate them here. See also our Github repository's scripts/09_cvm.tests.R
...
```{r}
cdp_tract <- readRDS("~/Dropbox/Mac/Desktop/nyu-capstone-F23-S24/data_build/cdp_tract.Rds")
summary(cdp_tract)
```
...
```{r}
cdp_tract_maj_bl <- readRDS("~/Dropbox/Mac/Desktop/nyu-capstone-F23-S24/data_build/cdp_tract_maj_bl.Rds")
...
```{r}
cdp_tract_pct_bl <- readRDS("~/Dropbox/Mac/Desktop/nyu-capstone-F23-S24/data_build/cdp_tract_pct_bl.Rds")
...
```{r}
cdp_tract_pct_wh <- readRDS("~/Dropbox/Mac/Desktop/nyu-capstone-F23-S24/data_build/cdp_tract_pct_wh.Rds")
summary(cdp_tract_pct_wh)
```

Combine into single results output table
...
cdp_table %>% 
  kable(
    digits = 2,
    caption = 'Table 5. Cramer von Mises Test Results',
    col.names = c('Covariates', 'Test Statistic', 'Critical Value', 'P-Value'),
    align = "l") %>% 
  add_footnote(c("Callaway and Sant'Anna propose the Cramer von Mises or Kolgorov-Smirnov-type integrated moments test as one tool for assessing whether the conditional parallel trends assumption might hold for all groups in all pre-treatment periods. See https://search.r-project.org/CRAN/refmans/did/html/conditional_did_pretest.html and Section 4 of Callaway & Sant'Anna (2020)."), notation = "none") %>% 
  kable_styling(full_width = F, html_font = "Roboto Condensed")
