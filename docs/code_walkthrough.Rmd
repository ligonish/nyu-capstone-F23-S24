...
tibble(v_codes, v_adm, v_desc) %>%
  kbl(col.names = c('Code', 'Law Violated ', 'HPD Description')) %>% 
...
*231,840 obs. of 19 vars* After trimming these to only include ZIP codes within New York City, using a c. 2010 ZIP/ZCTA list downloaded from NYC OpenData:
...
*216 obs. of 2 vars* We rename the variables more legibly and generate our covariates of interest as rates within each ZIP boundary for easier comparability across ZIPs.
...
violations %>% 
  ...
  kable(
    caption = 'NYC Right-to-Counsel Treatment Waves Observed in 2013 - 2019 Study Period',
...
violations %>% 
  ...
  st(group = 'cohort',
     group.test = T,
     col.align = 'center',
     title = 'Table 1. Summary Statistics by RTC Treatment Cohort: NYC Renter-Occupied Units, January 2012 - May 2019',
     out = 'kable') %>% 
  kable_styling(full_width = F, html_font = "Roboto Condensed")
```
...
cdp_table %>% 
  kable(
    digits = 2,
    caption = 'Cramer von Mises Test of Conditional Parallel Trends Assumption',
